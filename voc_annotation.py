...
#   默认指向根目录下的VOC数据集
#-------------------------------------------------------#
VOCdevkit_path      = 'F:/Project/JiShi/Ponding_Recognition/VOCdevkit/'
...
if __name__ == "__main__":
    ...
    for set in sets:
        ...
        for i in range(len(arr)):
            imgname = arr[i].split('\n')[0]
            print(imgname)
            image = Image.open(img_path + imgname + ".jpg")
            image.save(save_img_path + imgname + ".jpg")
            label = Image.open(label_path + imgname + ".png")
            label = np.asarray(label)
            label = Image.fromarray(np.uint8(label))
            label.save(save_label_path + imgname + ".png")